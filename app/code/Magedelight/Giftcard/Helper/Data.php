...
class Data extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    {
        return $this->getConfig('magedelight/general/enable_giftcard');
    }
    ...
    {
        return $this->_scopeInterface->getValue(
            'magedelight/general/enable_giftcard',
            \Magento\Store\Model\ScopeInterface::SCOPE_STORE
        );
    }
    ...
    public function isGiftcardForProducts()
    {
        $option = $this->_scopeInterface->getValue(
            'magedelight/general/giftcard_in_cart',
            \Magento\Store\Model\ScopeInterface::SCOPE_STORE
        );
        if (!$option) {
        ...
        } elseif (!$this->cart->getQuote()->getItemsCount()) {
            return false;
        }
        return true;
    }
    ...
    {
        $PriceBlock = $this->blockFactory->createBlock('Magento\Framework\View\Element\Template');
        $PriceBlock->setProduct($product);
        if ($product->getGiftcardPriceType() == '2') {
            $PriceBlock->setMinPrice($this->getFormattedPrice($product->getGiftcardPriceMin()));
            $PriceBlock->setMaxPrice($this->getFormattedPrice($product->getGiftcardPriceMax()));
        } else {
            $PriceBlock->setGiftcardBalance($this->getFormattedPrice($product->getGiftcardBalance()));
        }
        $PriceBlock->setTemplate('Magedelight_Giftcard::customPrice.phtml');
    ...
    /**
     * Check if gift card block can be displayed on cart and checkout page
     *
     * @return bool
     */
    public function isGiftcardAllowed()
    {
        if ($this->isActive() && $this->isGiftcardForProducts()) {
            return true;
        }
        return false;
    }
