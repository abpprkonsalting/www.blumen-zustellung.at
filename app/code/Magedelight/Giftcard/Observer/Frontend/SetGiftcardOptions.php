...
class SetGiftcardOptions implements ObserverInterface
{
    ...
    {
        if ($this->request->getFullActionName() == 'checkout_cart_add' || $this->request->getFullActionName() == 'checkout_cart_updateItemOptions' || $this->request->getFullActionName() == 'sales_order_reorder')
        {
            ...
            if ($product->getTypeId() == 'giftcard')
            {
                if(!empty($options)){
                    ...
                    $product->addCustomOption('additional_options', $this->serialize->serialize($additionalOptions));
                }
                else{
                    if($this->request->getFullActionName() == 'sales_order_reorder'){
                        throw new \ErrorException(__('Please specify product\'s required option(s);'));
                    }
                }
            }
        }
