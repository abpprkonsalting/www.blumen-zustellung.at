...
class SetGiftcardPrice implements ObserverInterface
{
    ...
    public function execute(Observer $observer)
    {
        if ($this->request->getFullActionName() == 'checkout_cart_add' || $this->request->getFullActionName() == 'checkout_cart_updateItemOptions') {
            ...
            $this->loadProduct = $this->productModel->create()->load($this->product->getId());
            $options = $this->request->getParam('additional_options');
            if (!empty($options) && $this->product->getTypeId() == 'giftcard') {
                $price = $this->getPriceByItem($this->loadProduct);
    ...
    public function getPriceByItem($product)
    {
        $options = $this->request->getParam('additional_options');
        $additionalOptions = [];
        foreach ($options as $key => $value) {
            if ($key == 'price') {
                if ($this->validateCustomPrice($value, $product)) {
                    $price = $value;
                } else {
    ...
    public function validateCustomPrice($price, $product)
    {
        $priceType = $product->getData('giftcard_price_type');
        if ($priceType == 1) {
