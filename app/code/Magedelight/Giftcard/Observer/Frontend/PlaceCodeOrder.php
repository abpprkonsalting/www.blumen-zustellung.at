...
class PlaceCodeOrder implements ObserverInterface
{
    ...
    public function execute(Observer $observer)
    {
        ...
        if ($customerId) {
            ...
            $customer = $orderInstance->getAddresses()[0];
            // $customer->getAddressType() = billing, if only gift card product added.
            // $customer->getAddressType() = shipping, if both gift card and other type of product added.
        ...
        foreach ($items as $item) {
            if ($item->getProductType() == Giftcard::TYPE_GIFTCARD_PRODUCT) {
                ...
                for ($i = 1; $i <= $item->getQtyOrdered(); $i++) {
                    ...
                    $itemsData['customer_name'] = $customerName;
                    $itemsData['customer_email'] = $customerEmail;
